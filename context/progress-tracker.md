...
- Vercel warned Node 20.x builds will fail starting 2026-10-01. Added `"engines": { "node": "24.x" }` to `package.json`. If Vercel's Project Settings → General → Node.js Version was ever set explicitly there, that dashboard value takes precedence over `engines` — needs checking/updating there too
- `/contact` returned 500 on every submit. Root cause confirmed by SMTP `verify()`: Gmail rejects the app password (`535-5.7.8 Bad Credentials`, `EAUTH`) — it was revoked on Google's side, the env var is present and well-formed. The 500 came from `sendEmail()` being the one un-caught step in `submitContactForm()` while the Supabase insert was already wrapped, so a mail outage failed a request whose message had *already* been stored. Email is now best-effort: the insert result is checked properly (supabase-js reports errors on the result, it does not throw, so the old code could not have detected a failed insert), and only a total loss — nothing stored *and* nothing sent — returns an error. Verified against a production build: POST `/api/contact` now returns 201 with the SMTP failure logged, where it previously 500'd. **Still needs a regenerated Gmail app password** to actually deliver mail
- `/admin` crashed with "An error occurred in the Server Components render". `requireAdmin()` throws a raw `Error`, and `/admin` is the only admin page that calls it during a *render* — every other admin page is a client component hitting an API route that catches it and returns 401, which is why only the dashboard broke. Split the guard: `requireAdmin()` (throwing) stays for API routes, new `requireAdminPage()` redirects instead and is used by `app/actions/admin-dashboard.ts`. Added `app/(admin)/error.tsx` — placed above `admin/layout.tsx` so it also catches layout failures, plain English since the admin group renders outside `NextIntlClientProvider`, and it surfaces `error.digest` so a production report can be matched to a Vercel log line. Not reproduced end-to-end: confirming which throw fires needs a valid admin session, so the underlying trigger should still be read off Vercel's runtime logs
